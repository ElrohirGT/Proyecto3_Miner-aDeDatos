...
# Asumiendo que train_imp ya existe y tiene la misma estructura que se usó para data_to_cluster
...
rpart.plot(modelo_arbol,
           type = 2,                # Tipo simplificado de visualización
           extra = 2,               # Muestra solo la predicción de clase (mucho menos información)
           box.palette = "RdYlGn",  # Esquema de colores
           branch.lwd = 1.2,        # Grosor de líneas reducido
           fallen.leaves = TRUE,    # Hojas alineadas
           under = TRUE,            # Texto bajo los nodos
           space = 0.5,             # Mucho más espacio entre nodos (aumentado significativamente)
           gap = 3,                 # Mayor espacio entre niveles
           compress = FALSE,        # No comprimir el árbol
           main = "Árbol de Clasificación para la diferencia de edad",
           cex = 0.8,               # Texto más pequeño en general
           tweak = 1.2,             # Ajuste fino de espaciado
...
# Configurar el cluster paralelo con el número de núcleos que ya definiste
cl <- makeCluster(ncores)
registerDoParallel(cl)

# Función modificada para tuning paralelo
tune_tree_depth_parallel <- function(max_depth_values, data) {
  # Crear un data frame para almacenar los resultados
  results <- foreach(depth = max_depth_values, .combine = rbind) %dopar% {
    library(rpart)
    library(caret)
    
    # Crear un control de parámetros con la profundidad específica
    control <- rpart.control(maxdepth = depth, 
                            minsplit = 20,
                            minbucket = 7,
                            cp = 0.01)
    
    # Generar los índices para validación cruzada
    set.seed(123 + depth)  # Semilla diferente para cada profundidad
    cv_folds <- createFolds(data$diferenciaEdad, k = 5, list = TRUE, returnTrain = FALSE)
    
    # Ejecutar la validación cruzada
    fold_results <- lapply(cv_folds, function(test_index) {
      train_fold <- data[-test_index, ]
      test_fold <- data[test_index, ]
      
      # Entrenar modelo con esta profundidad
      model <- rpart(diferenciaEdad ~ ., method = "class", 
                    data = train_fold, control = control)
      
      # Predecir en el conjunto de prueba
      predictions <- predict(model, newdata = test_fold, type = "class")
      
      # Calcular métricas
      conf <- confusionMatrix(predictions, test_fold$diferenciaEdad)
      return(c(conf$overall["Accuracy"], conf$overall["Kappa"]))
    })
    
    # Calcular promedio de métricas en los folds
    fold_results_matrix <- do.call(rbind, fold_results)
    mean_accuracy <- mean(fold_results_matrix[, 1])
    mean_kappa <- mean(fold_results_matrix[, 2])
    
    # Retornar los resultados para esta profundidad
    data.frame(max_depth = depth,
...
# Definir los valores de profundidad máxima a probar
max_depth_values <- c(5, 10, 15, 20, 25, 30)

# Ejecutar el tuning paralelo
system.time(
  tuning_results <- tune_tree_depth_parallel(max_depth_values, data_to_cluster)
)

# Detener el cluster cuando termines
stopCluster(cl)

# Ver resultados y continuar como antes
print(tuning_results)
```

| Profundidad| accuracy | kappa |
|-----------|----------|-------|
| 5 | 0.7238856 | 0.3951435 |
| 10 | 0.7238856 | 0.3951389 |
| 15 | 0.7238856 | 0.3951442 |
| 20 | 0.7238856 | 0.3951411 |
| 25 | 0.7238856 | 0.3951402 |
| 30 | 0.7238856 | 0.3951413 |

Se generaron 6 modelos más de árboles de clasificación, cambiando la profundidad del árbol, se puede observar qe tanto el kappa value como el accuracy no mejoran de manera significativa con respecto al modelo incial ya que el accuracy se mantiene en 0.72 y el kappa value en 0.39. Con esta información se puede concluir que el modelo de árbol de clasificación no es beneficioso para realizar predicciones sobre nuestra variable respuesta ya que tiene problemas en identificar las categorías establecidas. 
