...
Se analiza el vínculo existente entre víctima y agresor.

```{r}
rel_vic_agr <- original %>%
  group_by(VIC_REL_AGR) %>%
  summarise(cantidad = n()) %>%
  arrange(desc(cantidad))

etiquetas_relacion <- c(
  "1" = "Cónyuge", 
  "2" = "Ex-cónyuge",
  "3" = "Conviviente",
  "4" = "Ex-conviviente",
...
ggplot(rel_vic_agr, aes(x = reorder(factor(VIC_REL_AGR), -cantidad), y = cantidad)) +
  geom_bar(stat = "identity", fill = "darkgreen") +
  scale_x_discrete(labels = function(x) etiquetas_relacion[x]) +
  labs(title = "Frecuencia de denuncias según relación víctima-agresor",
       x = "Tipo de relación", y = "Número de denuncias") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

original %>%
  group_by(VIC_SEXO, VIC_REL_AGR) %>%
  summarise(cantidad = n()) %>%
  ggplot(aes(x = factor(VIC_REL_AGR), y = cantidad, fill = factor(VIC_SEXO))) +
  geom_bar(stat = "identity", position = "dodge") +
  scale_x_discrete(labels = function(x) etiquetas_relacion[x]) +
  scale_fill_manual(values = c("1" = "skyblue", "2" = "pink"),
                   labels = c("1" = "Hombre", "2" = "Mujer")) +
  labs(title = "Relación víctima-agresor según sexo de la víctima",
       x = "Tipo de relación", y = "Número de denuncias", fill = "Sexo de la víctima") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
